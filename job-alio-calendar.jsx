...
const gfd=(y,m)=>new Date(y,m,1).getDay();
const fmt=d=>{const x=new Date(d);return`${x.getMonth()+1}/${x.getDate()}`};
const inR=(d,s,e)=>{const a=new Date(d),b=new Date(s),c=new Date(e);[a,b,c].forEach(x=>x.setHours(0,0,0,0));return a>=b&&a<=c};
const dd=end=>{const t=new Date(),e=new Date(end);t.setHours(0,0,0,0);e.setHours(0,0,0,0);const d=Math.ceil((e-t)/864e5);return d<0?{t:"마감",u:false,x:true}:d===0?{t:"D-Day",u:true,x:false}:{t:`D-${d}`,u:d<=5,x:false}};
...
export default function App(){
  ...
  const load=useCallback(async()=>{
    setLd(true);
    // API_URL이 비어있거나 placeholder면 데모 모드
    if(!API_URL||API_URL.includes("your-app")){
      setJobs(DEMO);setDemo(true);setLu(new Date().toISOString());setLd(false);
      return;
    }
    try{
      ...
      console.warn("API 연결 실패, 데모 모드:",e.message);
  ...
  useEffect(()=>{
    load();
    ref.current=setInterval(()=>{const n=new Date();if((n.getUTCHours()+9)%24===9&&n.getMinutes()===0)load()},6e4);
  ...
  const DIM=dim(yr,mo),FD=gfd(yr,mo);
  const cells=[];for(let i=0;i<FD;i++)cells.push(null);for(let d=1;d<=DIM;d++)cells.push(d);
  const ts=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`;
  const act=fj.filter(j=>!dd(j.endDate).x);
  ...
  const pv=()=>{mo===0?(setYr(y=>y-1),setMo(11)):setMo(m=>m-1);setPn(false)};
  const nx=()=>{mo===11?(setYr(y=>y+1),setMo(0)):setMo(m=>m+1);setPn(false)};
  ...
  <style>{`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .sc::-webkit-scrollbar{width:3px}.sc::-webkit-scrollbar-thumb{background:rgba(70,120,240,.15);border-radius:2px}
  ...
  <div className="H">
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        {ld?<div className="sp"/>:(
          <><div className="pl" style={{width:5,height:5,borderRadius:"50%",background:demo?"#fbbf24":"#22c55e"}}/><span className="m" style={{fontSize:10,color:"#3a4e6e"}}>{demo?"DEMO":lu?`${new Date(lu).toLocaleTimeString("ko-KR",{hour:"2-digit",minute:"2-digit"})} 갱신`:""}</span></>
        )}
  ...
  <div style={{padding:"14px 14px 6px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:3}}>
      {DAYS_KR.map((d,i)=><div key={d} style={{textAlign:"center",fontSize:10.5,fontWeight:600,color:i===0?"rgba(248,113,113,.5)":i===6?"rgba(96,165,250,.5)":"#38485e",padding:"5px 0",letterSpacing:".5px"}}>{d}</div>)}
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
      {cells.map((day,i)=>{
        ...
        const ds=`${yr}-${String(mo+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
        const dj=jfd(ds);const isT=ds===ts,isS=sel?.ds===ds;
        ...
        return(
          <div key={day} className={`D ${isT?"t":""} ${isS?"s":""}`} onClick={()=>click(day)}>
            ...
            {dj.length>0&&<div style={{display:"flex",gap:1.5,alignItems:"center"}}>
